...
object Server {
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] =
    reframedFlow.collect { case value => Identity.parse(value) }.toMat(Sink.head)(Keep.right)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var buffer: List[Event] = List.empty[Event]
      var number = 1

      event => {
        if (event.sequenceNr > number) {
          buffer = event :: buffer
          Nil
        } else {
          val sortedBuffed = (event :: buffer).sortBy(_.sequenceNr)
          number = if (buffer.isEmpty) event.sequenceNr + 1 else buffer.last.sequenceNr + 1
          buffer = Nil
          sortedBuffed
        }
      }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var map: Followers = Map.empty[Int, Set[Int]].withDefaultValue(Set.empty[Int])

      event => {
        event match {
          case Follow(_, fromUserId, toUserId) =>
            map += (fromUserId -> (map(fromUserId) + toUserId))
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers._1 match {
      ...
      case StatusUpdate(_, fromUserId) => eventAndFollowers._2.get(userId).exists(_.contains(fromUserId))
...
class Server()(implicit executionContext: ExecutionContext, materializer: Materializer)
  ...
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut.collect {
      case (event, eventAndFollowers) if isNotified(userId)(event -> eventAndFollowers) =>
        event.render
    }
